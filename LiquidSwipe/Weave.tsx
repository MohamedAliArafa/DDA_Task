...
import Svg, { ClipPath, Path, Defs } from "react-native-svg";
...
// Prepare Animated Path variable for the Masked View
...
export default ({
  ...
  //Android snack bar hight
  const snackBarHeight = 100;

  //Build up SVG Path Commands using Helpers to get a SVG like path like "M21,65,56L0,65,"
  const commands: Animated.Node<string>[] = [];
  moveTo(commands, sub(maskWidth, sideWidth), 0);
  lineTo(commands, 0, 0);
  lineTo(commands, 0, height + snackBarHeight);
  lineTo(commands, maskWidth, height + snackBarHeight);
  ...
  return (
    ...
    // Masked View is the Alternitave for MaskedViewIOS with Android Support
